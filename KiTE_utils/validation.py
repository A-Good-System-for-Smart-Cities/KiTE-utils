def check_attributes(X, e, iterations=1000, n_jobs=1):
    ...
    if X.shape[0] != e.shape[0]:
        raise ValueError(
            "Incompatible dimension for X and e matrices. X and e should have the same feature dimension,"
            f": X.shape[0] == {X.shape[0]} while e.shape[0] == {e.shape[0]}."
        )
    ...
    if not (isinstance(n_jobs, int) and n_jobs > 0):
        raise ValueError(f"n_jobs is incorrect type or <1. n_jobs: {n_jobs}")
